...
{
    public class VideosImages : IRemoteImageProvider
    {
        public string Name => Plugin.Instance.Name;

        public bool Supports(BaseItem item) => item is Video && !(item is MediaBrowser.Controller.Entities.Movies.Movie);
...
#endif
        {
            ...
            try
            {
                images = await StashAPI.SceneImages(curID, cancellationToken).ConfigureAwait(false);
            }
            catch (Exception e)
            {
