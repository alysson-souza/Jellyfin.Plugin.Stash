...
#endif
            => PlaybackSync.Instance?.Import(progress, cancellationToken) ?? Task.CompletedTask;
    }
...
#endif
        {
            if (PlaybackSync.Instance != null)
            {
                await PlaybackSync.Instance.Reconcile(cancellationToken).ConfigureAwait(false);
                Helpers.Logger.Info(PlaybackSync.Instance.Status);
            }
